...
class PromotionEvent(Base):
    ...
    products = relationship(
        "Product",
        # secondary is the association table that links Product and PromotionEvent (linking table)
        secondary="product_promotion_event",
        # back_populates is used to define the reverse side of the relationship in the Product class
...
class Product(Base):
    ...
    promotion_event = relationship(
        ...
        # secondary is the association table that links Product and PromotionEvent (linking table)
        secondary="product_promotion_event",
        # back_populates is used to define the reverse side of the relationship in the PromotionEvent class
...
class ProductPromotionEvent(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    # Establishing many-to-many relationship between Product and PromotionEvent
    product_id = Column(
    ...
    # id producti_id promotion_event_id
    # 1   10            2
    # 2   10            3
    # 3   11            2

    # we want to ensure that the same product is not linked to the same promotion event more than once
    # so we add a unique constraint on the combination of product_id and promotion_event_id
    __table_args__ = (
        UniqueConstraint(
